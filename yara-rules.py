...
class YaraRules:
    ...
    def get_zip(self, url):
        ...
        if response.status_code == 200:
            ...
            raise UrlError(f'code:{response.text}, text:{response.text}')
        return rules
...
def main(args):
    ...
    try:
        ...
        sys.exit(f'failed to find configured base dir: {BASE_DIR}')
    ...
    for dirpath, dirnames, filenames in os.walk(BASE_DIR, followlinks=True):
        for filename in filenames:
            if filename.endswith((".yar", ".yara")):
                index_folder.add(f'include "{os.path.join(dirpath, filename)}"')
